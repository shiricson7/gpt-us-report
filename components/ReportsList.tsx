...
export default function ReportsList(props: { initialRows: ReportsListRow[] }) {
  const router = useRouter();
  const [rows, setRows] = useState<ReportsListRow[]>(props.initialRows);
  const [deleting, setDeleting] = useState<Record<string, boolean>>({});
  const [error, setError] = useState<string | null>(null);

  async function deleteRow(row: ReportsListRow) {
    ...
    try {
      const res = await fetch(`/api/reports/${row.id}`, { method: "DELETE" });
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/60 bg-white/70 shadow-sm backdrop-blur">
      ...
      {error ? (
        <div className="border-b border-white/60 bg-rose-50 px-4 py-2 text-xs text-rose-700">
          {error}
        </div>
      ) : null}
      ...
      {rows.length ? (
          ...
          {rows.map((r) => (
            <div key={r.id} className="group relative grid grid-cols-12 gap-2 px-4 py-3 text-sm">
              <Link
                ...
                aria-label={`Open report: ${r.patientName || "Untitled"}`}
                className="absolute inset-0 rounded-none focus:outline-none focus:ring-2 focus:ring-slate-900/20"
              />

              <div className="pointer-events-none col-span-4 font-medium group-hover:text-slate-900">
              ...
              <div className="col-span-1 flex justify-end">
                <button
                  type="button"
                  title="Delete"
                  aria-label={`Delete report: ${r.patientName || "Untitled"}`}
        ...
        <div className="px-4 py-10 text-center text-sm text-slate-600">No reports yet.</div>
      )}
    </div>
  );
}
